...
public class ServerGame implements Screen {
    ...
    public ServerGame(String name, boolean isServer) {
        this.name = name;
        try {
    ...
    public void show () {
        ...
        FileHandle handle = Gdx.files.local("maps/map.afm");
        String data = handle.readString();
        String lines[] = data.split(";");
        FileHandle mdfile = Gdx.files.local("maps/map_data.afmd");
        ...
        for (int x = 0; x < mapData.MAP_X; x++) {
            for (int y = 0; y < mapData.MAP_Y; y++) {
                ...
                mapInformation.add(new TileInformation(Integer.valueOf(fieldData[0].replace("\r\n", "")), Boolean.valueOf(fieldData[1]), texturen, x, y));
            }
        }

        player = new Player(0, 0, skin, name, texturen);
        ...
        if (isServer)
            server = new Server(1339, texturen);

        player = new Player(0, 0, Integer.valueOf(tmpLinesofSt[0]), this.name, texturen);
